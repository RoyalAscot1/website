...
The backend requires a running PostgreSQL instance. Connection string is in `api/.env` as `DATABASE_URL`. The database and tables are auto-created on startup via SQLAlchemy `metadata.create_all(engine)`.
...
Routers:
- `POST /upload-CSV` — parses CSV, enriches via Yahoo Finance, persists snapshot + investments
- `POST /upload-survey` — persists survey answers
...
- `POST /recommendations` — calls `analyze_snapshot()` internally, fetches survey, then sends both to **Gemini 2.5 Pro** (`google-genai` SDK) for a structured narrative + 4 bullet recommendations
